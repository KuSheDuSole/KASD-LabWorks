...
namespace RBTreeLib
{
    ...
    public class RBTree<K>: MyNavigableSet<K> where K: IComparable
    {
        public class MyItr : MyIterator<K>
        {
            public K? Cursor { get; set; }
            private int index;
            private RBTree<K> tree;
            private K[] array;
            public MyItr(RBTree<K> rbt)
            {
                tree = rbt;
                array = tree.ToArray();
                index = 0;
        ...
        public K[] SubSet(K start, K end)
        {
            ...
            K[] mas = returnTree.ToArray();
            return mas;
        }
        public K[] HeadSet(K start)
        {
            SortedSet<K> returnTree = new SortedSet<K>();
            Node step = root!;
            Stack<Node> stack = new Stack<Node>();
        ...
        public K? LowerEntry(K key) => Lower(key);
        public K? FloorEntry(K key) => Floor(key);
        public K? HigherEntry(K key) => Highter(key);
        public K? CeilingEntry(K key)=> Ceiling(key);
        public K? LowerKey(K key) => Lower(key);
        public K? FloorKey(K key) => Floor(key);
        public K? HigherKey(K key) => Highter(key);
        public K? CeilingKey(K key) => Ceiling(key);
        public K? PollFirstEntry() => PollFirst();
        public K? PollLastEntry() => PollLast();
        public K? FirstEntry() => First();
        public K? LastEntry() => Last();
