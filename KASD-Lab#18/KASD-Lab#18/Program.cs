namespace MyHashMapLib
{
    public class MyHashMap<K, V>
    {
        ...
        {
            KeyValuePair<K, V>[] array = new KeyValuePair<K, V>[size];
            int index = 0;
            foreach (var element in table)
            {
                for (var step = element; step != null; step = step.next)
                    array[index++] = new KeyValuePair<K, V>(step.key, step.value);
            }
            return array;
        }
        public void Print()
    ...
    //    {
    //        MyHashMap<string, int> myHashMap = new MyHashMap<string, int>(6);
    //        myHashMap.Put("a", 1);
    //        myHashMap.Put("b", 2);
    //        myHashMap.Put("c", 3);
    //        myHashMap.Put("d", 4);
